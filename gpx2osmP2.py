...
#  log.debug(u"parsed_len=%d"%parsed_len)
  if len(parsed)<2:
    return None
  else:
    removed=parsed[0:parsed_len-1]
    log.debug(u"1: removed=")
    log.debug(removed)
    if parsed_len>=3:
...
def check_symbol_type(s):
  ...
  if type(s) is int:
    ...
    if s.isdigit():
      index_type=0
    elif re.search(u'[а-я]',s.lower()) !=None or re.search(u'[a-z]',s.lower()) !=None:
      index_type=1
    else:
      index_type=-1
  return index_type
  ...
  for way_id in ways:
    ...
    for cur_node_id in check_list:
      if check_connected(ways,way_id,cur_node_id) == False or selected_begin == True:
        ...
        log.debug(u"try connect poi with ref=%s"%poi[cur_node_id]["name"])
        new_ref=remove_index(poi[cur_node_id]["name"])
        log.debug(u"try find new_ref=%s"%new_ref)
        # ищем такую опору:
        for node_id in poi:
          if poi[node_id]["name"] == new_ref:
            log.debug(u"found ref=%s"%new_ref)
            ...
            if node_id not in way:
              ...
              log.debug(u"dist=%f"%dist)
              if dist < MAX_DIST:
                log.debug(u"not far - accept node!")
...
def get_prefery_next_ref(ref,inc):
  log.debug(u"get_prefery_next_ref(%s,%d)"%(ref,inc))
  parsed=parse_ref(ref)
  if parsed==None:
  ...
  if last_type == 0:
    ...
    new_index_symbol=unichr(ord(index_symbol)+inc)
...
#    log.debug(u"index=%d"%index)
#    log.debug(u"s=%s"%ref[index])
#    log.debug(u"cur_block_type=%d"%cur_block_type)
#    log.debug(u"cur_symbol_type=%d"%cur_symbol_type)
...
def get_next_candidat(poi,last_point,list_newarest_points,cur_name):
  log.debug(u"get_next_candidat()")
  prefer_next_name=get_prefery_next_ref(cur_name,1)
  prefer_prev_name=get_prefery_next_ref(cur_name,-1)
  
  log.debug(u"предполагаемые следующие имена: '%s' и '%s'"%(prefer_next_name,prefer_prev_name))
  log.debug(u"Ближайшие точки к poi c именем '%s' по расстоянию:"%cur_name)
           
  candidat=None
  for poi_id in list_newarest_points:
    log.debug(u"%s"%poi[poi_id]["name"])
    if poi[poi_id]["name"]==prefer_next_name or poi[poi_id]["name"]==prefer_prev_name:
      # ищем ближайшую опору с предполагаемым именем:
      candidat=poi[poi_id]
      log.debug(u"нашли ближайшую подходящую по имени с ref=%s"%candidat["name"])
      break;
  #if candidat==None:
...
def get_begin_candidat(poi,last_point,list_all_newarest_points,cur_name):
  log.debug(u"get_begin_candidat()")
  begin_ref=get_prefery_begin(cur_name)
  if begin_ref==None:
    log.debug(u"не смог вычислить начальное имя для '%s' - пропуск"%cur_name)
    return None
    
  log.debug(u"предполагаем следующее имя начала: '%s'"%begin_ref)
  ...
  for poi_id in list_all_newarest_points:
    ...
    if poi[poi_id]["name"]==begin_ref:
      ...
      break;
  if candidat==None:
    log.debug(u"по имени не нашли - пропуск")
  else:
    log.debug(u"candidat ref=%s"%candidat["name"])
  return candidat
...
def create_line(poi):
  ...
  for poi_id in poi:
    ...
    log.debug(u"process poi_d=%s"%poi_id)
    log.debug(u"process poi ref=%s"%item["name"])
    ...
    if "way_id" not in item:
      ...
      while True:
        ...
        log.debug(u"=========  Ищем ближайшие точки для: '%s'"%cur_name)
        list_newarest_points=get_nearest_points(last_point["lat"], last_point["lon"],poi,MAX_DIST)
        log.debug(u"len(list_newarest_points)=%d"%len(list_newarest_points))
        for i in list_newarest_points:
          log.debug(u"id:%d, ref:%s"%(i,poi[i]["name"]))
        ...
        if dist>MAX_DIST or end_line==True:
          ...
          log.debug(u"пробуем прикрепить к началу:")
          ...
          log.debug(u"добавляем точку с ref='%s' как следующую для точки с ref='%s'"%(candidat["name"],cur_name))
          if processed_flow==1:
            ways[way_id].append(candidat["poi_id"])
          else:
            ways[way_id].insert(0,candidat["poi_id"])
...
def parse_file_name(path_name):
  ...
  log.debug(u"path input file=%s"%path_name.decode('utf8'))
  name=os.path.basename(path_name).strip().decode('utf8')
  log.debug(u"file name=%s"%name)

  if "_line." in name and re.search(u'^вл[ -_]',name.lower()) != None:
    result={}
    # vl
    words=name.split(' ')
    if words[1].isdigit():
    ...
    result["power"]="line"
  elif "_line." in name and (re.search(u'^квл[ -_]',name.lower()) != None or re.search(u'^кл ',name.lower()) != None):
    result={}
    # vl
    words=name.split(' ')
    if words[1].isdigit():
    ...
    # minor_line
  elif "_station." in name and re.search(u'^пс[ -_]',name.lower()) != None:
    # ps
    result={}
    words=name.split(' ')
    ...
    if words[1].isdigit():
      ...
      voltage_str=re.sub(u'_.*','',words[1])
...
#  log.addHandler(fh)
  ...
  try:
    ...
    sys.exit(1)
  log.debug(u"max_dist=%s"%args.max_dist)
