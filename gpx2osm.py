...
def parse_file_name(path_name):
  ...
  if \
      log.info("пробуем найти '_line.' в имени файла линии: '%s'"%name) or \
      "_line." in name:
    log.info("УСПЕХ! нашли '_line.' в имени файла линии: '%s'"%name) 
    if \
        log.info("пробуем найти в начале имени 'вл ' (с пробелом после 'вл') в имени линии, приведённому к нижнему регистру: '%s'"%name.lower()) or \
        re.search(r'^вл ',name.lower()) != None:
      log.info("УСПЕХ! нашли 'вл ' (с пробелом после 'вл') в начале имени линии, приведённому к нижнему регистру: '%s'"%name.lower())
      result={}
      # vl
      words=name.split(' ')
      if words[1].isdigit():
      ...
      result["power"]="line"
        ...
        log.info("поробуем найти 'квл ' (с пробелом после 'квл') или 'кл ' (с пробелом после 'кл') в начале имени линии, приведённому к нижнему регистру: '%s'"%name.lower()) or \
        re.search(r'^квл ',name.lower()) != None or re.search(u'^кл ',name.lower()) != None:
      log.info("УСПЕХ! нашли 'квл ' (с пробелом после 'квл') или 'кл ' (с пробелом после 'кл') в начале имени линии, приведённому к нижнему регистру: '%s'"%name.lower())
      result={}
      # vl
      words=name.split(' ')
      if words[1].isdigit():
      ...
      log.error("ОШИБКА! имя файла некорректно сформировано! Несмотря на то, что '_line' было найдено в линии - требуется найти корректный префикс ('ВЛ ', 'КЛ ', 'КВЛ ' - с пробелами после префикса!), однако этого найдено не было - выход!") 
      
  elif \
      log.info("пробуем найти '_line04.' в имени файла линии: '%s'"%name) or \
      "_line04." in name:
    log.info("УСПЕХ! нашли '_line04.' в имени файла линии: '%s'"%name) 
    result={}
    result["voltage"]=400
    ...
    # minor_line
      ...
      log.info("пробуем найти '_station.' в имени файла: '%s'"%name) or \
      "_station." in name and re.search(r'^пс ',name.lower()) != None:
    # ps
    log.info("УСПЕХ! нашли '_station.' в имени файла: '%s'"%name) 
    result={}
    words=name.split(' ')
    ...
    result["power"]="station"
      ...
      log.info("пробуем найти '_substation.' в имени файла линии: '%s'"%name) or \
      "_substation." in name:
    log.info("УСПЕХ! нашли '_substation.' в имени файла линии: '%s'"%name) 
    #tp
    result={}
    result["power"]="sub_station"
  else:
    log.error("ОШИБКА! имя файла некорректно сформировано! Требуется перепроверить имя файла, чтобы оно содержало правильное наименование") 
    # error
